...
-- 04: Which city has the best customers? We would like to throw a promotional Music Festival in the city 
-- we made the most money. Write a query that returns one city that has the highest sum of invoice totals.
--  Return both the city name & sun of all invoice totals
...
SELECT DISTINCT email, first name, last_name
FROM customer
JOIN invoice ON customer.customer_id = invoice.customer_id
...
SUM(invoice_line.unit_pricex*invoice_line.quantity) AS total_sales 
FROM invoice_Line
...
WHERE sales per country.purchases_per_genre = max_genre Jper_country.max_genre_number
...
SELECT customer.customer_id, first_name, last_name, billing_country, SUM(total) AS total spending FROM invoice
JOIN customer ON customer.customer_id = invoice.customer_id
GROUP BY 1,2,3,4
ORDER BY 2,3 DESC),
...
SELECT cc.billing_country, cc.total_spending, cc.first_name, cc.last_nam 
FROM customter_with_country cc
JOIN country_max_spending as
ON cc.billing_country = ms.billing_country
WHERE cc.total spending= ns.max_spending
ORDER BY 1;
